...
install: build
	$(R_DYNTRACE) CMD INSTALL --preclean --with-keep.source evaldyntracer*.tar.gz

clean:
	rm -rf evaldyntracer*.tar.gz
	rm -rf *.Rcheck
	rm -rf src/*.so
	rm -rf src/*.o
...
check: build
	$(R_DYNTRACE) CMD check --output=$(R_CMD_CHECK_OUTPUT_DIRPATH) evaldyntracer_*.tar.gz
